...
public class CompactModel {
  ...
  private StarRoutingSolution solve(boolean integral, Duration timeout) {
    ...
    try {
      ...
      StarRoutingSolution.Status status =
          IloCplex.Status.Optimal.equals(cplex.getStatus())
              ? StarRoutingSolution.Status.OPTIMAL
              : StarRoutingSolution.Status.TIMEOUT;
      List<Route> paths =
          integral && StarRoutingSolution.Status.OPTIMAL.equals(status)
              ? getPathsFromSolution()
              : new ArrayList<>();
      StarRoutingSolution solution =
          new StarRoutingSolution(status, cplex.getObjValue(), paths, elapsedTime);
      ...
      return new StarRoutingSolution(
          StarRoutingSolution.Status.TIMEOUT, 0.0, new ArrayList<>(), Utils.getElapsedTime(start));
    }
  }
  ...
  private void createVariables(boolean integral) throws IloException {
    ...
    for (int s = 0; s < S; s++) {
      for (int k = 0; k < K; k++) {
        y[s][k] =
            integral ? cplex.boolVar("y_" + s + "_" + k) : cplex.numVar(0, 1, "y_" + s + "_" + k);
      }
    }
    u = new IloIntVar[N][K];
    for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < N; j++) {
        for (int k = 0; k < K; k++) {
          if (i != instance.getDepot() && j != instance.getDepot() && j != i) {
            IloNumExpr mtz =
                cplex.sum(u[i][k], cplex.negative(u[j][k]), cplex.prod(x[i][j][k], N - 1));
