...
public class Experiments {
  ...
  private static void experiment1_compactModelPerformance() {
    Table table =
        new Table(
            List.of("Instancia", "|N|", "|S|", "|K|", "Tiempo (ms)", "#Ticks"),
            true,
            "experiment1.csv");
  ...
  private static void experiment41_monoLabelSettingPricingPerformance() {
    Table table =
        new Table(
            List.of(
                "Instancia",
  ...
  private static void experiment45_labelSettingHeuristicsPerformance() {
    Table table =
        new Table(
            List.of(
                "Instancia",
  ...
  private static void experiment5_labelSettingHeuristics() {
    ...
    for (Instance instance : InstanceLoader.getInstance().getExperimentInstances()) {
      ...
      table.addEntry(
          new ComparisonTableEntry(
              instance, solution1, solution2, columnGenerator1, columnGenerator2));
  ...
  private static void experiment8_relaxationComparison() {
    ...
    for (Instance instance : InstanceLoader.getInstance().getExperimentInstances()) {
      ...
      table.addEntry(
          new RelaxationComparisonTableEntry(instance, solution1, solution2, columnGenerator));
      if (solution1.timedOut() && solution2.timedOut()) {
        unfinishedInstances++;
      } else {
        unfinishedInstances = 0;
  ...
  private static String getElapsedTime(StarRoutingSolution solution) {
    return solution.timedOut()
        ? TIME_LIMIT_EXCEEDED
        : String.valueOf(solution.getElapsedTime().toMillis());
  }

  private static String getDeterministicTime(StarRoutingSolution solution) {
    return solution.hasDeterministicTime()
        ? FORMATTER.format(solution.getDeterministicTime())
        : DEFAULT_FIELD;
  }
  ...
  private static String getGapBetweenSolutions(
      StarRoutingSolution solution1, StarRoutingSolution solution2) {
    return !solution1.timedOut() && !solution2.timedOut()
        ? FORMATTER.format(gapAsPercent(solution1.getObjValue(), solution2.getObjValue())) + "%"
        : DEFAULT_FIELD;
  }

  private static String getGapToLowerBound(StarRoutingSolution solution) {
    return solution.hasLowerBound()
        ? FORMATTER.format(gapAsPercent(solution.getObjValue(), solution.getLowerBound())) + "%"
        : DEFAULT_FIELD;
  }
  ...
  private static class SimpleTableEntry implements Table.Entry {
    ...
    public List<String> getFields() {
      return List.of(
          ...
          getNumberOfNodes(instance),
          getNumberOfCustomers(instance),
  ...
  private static class ComparisonTableEntry implements Table.Entry {
    ...
    public ComparisonTableEntry(
        Instance instance,
        StarRoutingSolution solution,
        StarRoutingSolution solution2,
        ColumnGenerator columnGenerator1,
        ColumnGenerator columnGenerator2) {
      this.instance = instance;
    ...
    public List<String> getFields() {
      return List.of(
          ...
          getNumberOfCustomers(instance),
          getNumberOfVehicles(instance),
          getElapsedTime(solution1),
          getObjValue(solution1),
          getNumberOfIterations(columnGenerator1),
          getElapsedTime(solution2),
          getObjValue(solution2),
          getNumberOfIterations(columnGenerator2),
          getGapBetweenSolutions(solution2, solution1),
          getLowerBound(solution2),
          getGapToLowerBound(solution2));
    }
  }
  ...
  private static class RelaxationComparisonTableEntry implements Table.Entry {
    ...
    public RelaxationComparisonTableEntry(
        Instance instance,
        StarRoutingSolution solution,
        StarRoutingSolution solution2,
        ColumnGenerator columnGenerator) {
